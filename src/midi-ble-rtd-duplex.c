...
static bool duplex_alsa_event_is_midi_payload(const snd_seq_event_t *ev) {
    switch (ev->type) {
...
static const char *duplex_alsa_event_type_name(int type) {
    switch (type) {
...
static gboolean duplex_alsa_rx_poll_cb(gpointer user_data) {
    ...
    for (;;) {
        ...
        if (!duplex_alsa_event_is_midi_payload(ev)) {
            if (app->cfg.print_midi_events)
                g_print("Ignoring ALSA Sequencer control event type %d (%s).\n",
                        ev->type, duplex_alsa_event_type_name(ev->type));
            g_mutex_unlock(&dapp->alsa_lock);
            continue;
        }
